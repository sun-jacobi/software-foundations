...
Proof.
  intros n H. 
  inversion H as [|n' H0 Heq].
  inversion H0 as [|n'' H1 Heq1].
  apply H1.
Qed.
...
Proof.
  intros n m E Em. induction E as [|n' E' IH]. 
  - (* E = ev_0 *)
    apply Em.
  - (* E = ev_SS n' E'*)
...
Proof.
  intros n. split.
  - (* -> *)
    intros E. induction E as [ | |n' m' E' IHn IHn' IHm].
    + apply ev_0.
    + apply (ev_SS 0 ev_0).
...
Proof.
  intros n m E En. induction En as [|n' En' IH]. 
  - (* En = ev_0 *) apply E.
  - (* En = ev_SS n' En' with IH : ev (n' + m) -> ev m *) 
    apply IH. apply evSS_ev. apply E.
Qed.
...
Proof.
  intros n m p. intros Enm Enp. 
  assert (H: ev (n + n + (m + p))).
  - rewrite add_assoc. (* (((n + n) + m) + p) *)
    rewrite <- add_assoc with n n m. (* (n + (n + m) + p) *)
    rewrite add_comm with n (n + m).  (* (n + m  + n + p) *)
    rewrite <- add_assoc with (n + m) n p. (* (n + m + (n + p)) *)
...
Inductive total_relation : nat -> nat -> Prop :=
  | total_relation_n (n : nat)      : total_relation n 0
  | total_relation_s (n m : nat) (H : total_relation n m) : total_relation n (S m)
.

Theorem total_relation_is_total : forall n m, total_relation n m.
Proof.
  intros n m. induction m as [|m' IHm].
  - (* m = 0 *) apply total_relation_n.
  - (* m = (S m') *) apply total_relation_s. apply IHm.
Qed.
...
Proof.
  intros n m. intros contra. inversion contra.
Qed.
...
Proof.
  intros n. induction n as [|n' IHn].
  + (* n = 0 *) apply le_n.
  + (* n = S n' with IHn: 0 <= S n' *) apply le_S. apply IHn. 
Qed.

Theorem n_le_m__Sn_le_Sm : forall n m,
  n <= m -> S n <= S m.
Proof.
  intros n m E. induction E as [|m' E' IH].
  + apply le_n.
  + apply le_S. apply IH.
Qed.
...
Proof.
  ...
  - left. apply O_le_n.
  - destruct p.
    * right. apply plus_le in H. destruct H as [HL HR].
      simpl in HR. apply HR.
    * intros q H. simpl in H. rewrite plus_n_Sm in H. rewrite plus_n_Sm in H.
...
Proof.
  ...
  - simpl. apply H.
  - rewrite add_comm with (n := S p) (m := n).
    rewrite <- plus_n_Sm with (n := n) (m := p).
    rewrite add_comm with (n := S p) (m := m).
    rewrite <- plus_n_Sm with (n := m) (m := p).
    apply n_le_m__Sn_le_Sm.
    rewrite add_comm with (n := n) (m := p).
    rewrite add_comm with (n := m) (m := p).
    apply IHp.
Qed.
...
Proof.
  intros n1 n2 m H. induction H.
  - split. 
    + rewrite add_comm. rewrite plus_n_Sm. rewrite add_comm. apply le_plus_l.
    + rewrite plus_n_Sm. rewrite add_comm. apply le_plus_l.
  - destruct IHle.
    apply le_trans with (S n1) (m) (S m) in H0.
    apply le_trans with (S n2) (m) (S m) in H1.
    split. apply H0. apply H1. apply le_S. apply le_n. apply le_S. apply le_n.
Qed.
...
Proof.
  ...
  - destruct n.
    ...
    + intros H. simpl in H. apply IHm in H.
...
Proof.
  ...
  - intros n H. destruct n.
    ...
    + inversion H.
  - intros n H. destruct n.
    + apply IHm with (n := 0). apply O_le_n.
...
(*
  ...
  - No. Starting from the c1 and using c2 and c3, we can represent all additions.
...
Theorem R_equiv_fR : forall m n o, R m n o <-> fR m n = o.
  ...
  - (* R m n o -> m + n = o. *)
   intros H. induction H.
    ...
    + (* 
        IH: R m n o -> fR m n = o 
        Prove: R (S m) n o -> (S m) + n = o 
      *)
      simpl. rewrite IHR. reflexivity.
    + (* 
        IH: R m n o -> fR m n = o 
        Prove: R m (S n) o -> m + (S n) = o 
      *)
    rewrite <- plus_n_Sm. unfold fR in IHR. rewrite IHR. reflexivity.
    + (*
        IH: R (S m) (S n) (S (S o)) -> (S m) + (S n) = (S (S o))
        Prove: R m n o -> m + n = o 
      *)
  ...
  - (* fR m n = o -> R m n o *)
    ...
    + induction n. 
      * intros m H. 
        destruct m. { apply c1. } { discriminate H. }
      * intros m H. rewrite add_comm in H. discriminate H.
    + induction n.
      * intros m H. rewrite add_0_r in H. rewrite H. apply c2. 
        apply IHo. rewrite add_0_r. reflexivity.
      * intros m H. rewrite add_comm in H. simpl in H. 
        apply S_injective with (n + m) o in H.
        { apply c3. apply IHo. rewrite add_comm. apply H. }
Qed. 
...
| s2 l1 l2 (H: subseq l1 l2) (x: nat): subseq l1 (x :: l2)
| s3 l1 l2 (H: subseq l1 l2) (x: nat): subseq (x :: l1) (x :: l2)
.

Theorem subseq_1 : subseq [1;2;3] [1;1;1;2;2;3].
  apply s3. apply s2. apply s2. apply s3. apply s2. apply s3. apply s1. 
Qed.
...
Theorem R'_1 : R' 2 [1;0]. 
  apply c2. apply c2. apply c1.
Qed.

Theorem R'_2 : R' 1 [1;2;1;0].
...
Lemma MUnion' : forall T (s : list T) (re1 re2 : reg_exp T),
  ...
  s =~ Union re1 re2.
Proof.
  intros T s re1 re2. intros [HL | HR]. 
...
Lemma MStar' : forall T (ss : list (list T)) (re : reg_exp T),
  ...
  fold app ss [] =~ Star re.
Proof.
  intros T ss re H. induction ss as [|h ss' IH].
  - (* ss = [] *) 
    simpl. apply MStar0.
  - (* ss = [h::ss'] *)
    apply MStarApp. 
    + (* h =~ re *)
      apply H. unfold In. left. reflexivity.
    + apply IH. intros s H'. apply H. right. apply H'. 
Qed.
...
Proof.
  intros T re. split.
  - (* -> *)
    intros H. destruct H as [x Hmatch] .
    induction Hmatch
    as [| x'
        | s1 re1 s2 re2 Hmatch1 IH1 Hmatch2 IH2
        | s1 re1 re2 Hmatch IH | re1 s2 re2 Hmatch IH
    ...
    + (* MUnionR *) 
      simpl. rewrite IH. apply orb_true_iff. right. reflexivity.
    + (* MStar0 *)
      reflexivity.
    + (* MStarApp *)
      reflexivity.
  - intros H. induction re.
    ...
    + simpl in H. apply andb_true_iff in H. destruct H as [ H'].
      apply IHre1 in H'. apply IHre2 in H. destruct H'. destruct H.
      exists (x ++ x0). apply MApp. apply H0. apply H. 
...
Proof.
  ...
  - (* MStarApp *)
    destruct IH2. apply Heqre'.
    destruct H. exists (s1 :: x).
    split.
    + simpl. rewrite H. reflexivity.
    + intros s' H1. destruct H1.
      * rewrite <- H1. inversion Heqre'. rewrite <- H3. apply Hmatch1.
      * apply H0. apply H1.
Qed.
  ...
  *)
  ...
  - (*
    ...
    *)
    ...
    destruct H as [H1 | H2].
      + (* pumping_constant re1 <= length s1 *)
        apply IH1 in H1. destruct H1 as [x [y [z [H2 [H3 H4]]]]].
        (*
          Now we have x, y, z s.t
          s1 = x ++ y ++ z /\
          y != [ ] /\
          forall m : nat, x ++ napp m y ++ z =~ re1
        *)
...
Proof.
  ...
  - (*
    ...
    *)
    ...
    apply add_le_cases in H. 
    destruct H as [H1 | H2].
    + apply IH1 in H1. destruct H1 as [x [y [z [H2 [H3 [H4 H5]]]]]].
      exists x. exists y. exists (z ++ s2).
      split. rewrite H2. rewrite <- app_assoc. rewrite <- app_assoc. reflexivity.
      split. apply H3.
    ...
    - (* MMUnionL:  *) 
      ...
      destruct H as [HL HR].
      apply IH in HL.
      destruct HL as [x [y [z [H1 [H2 [H3 H4]]]]]].
      exists x. exists y. exists z.
      split. apply H1.
      split. apply H2.
...
Proof.
  ...
  - split.
    + (* P -> true = false *)
      intros H'. apply ex_falso_quodlibet. apply H. apply H'.
...
Proof.
  ...
  - (* l = m :: l' *) 
    ...
    + (* n = m *)
      intros _. rewrite H in Hcount. discriminate Hcount.
    + (* n <> m *)
      intros H'. destruct H'.
      * apply H. symmetry in H0. apply H0. 
      * apply IHl'. simpl in Hcount. apply Hcount. apply H0.
Qed.
...
| n1 : nostutter []
| n2 (x : X) : nostutter [x]
| n3 (l : list X) (x: X) (y: X): nostutter (x::l) -> x <> y -> nostutter (y::(x::l)).
...
| m1 : merge [] [] [] 
| m2 l l1 l2 (H: merge l1 l2 l) (x: X): merge (x::l1) l2 (x::l)
| m3 l l1 l2 (H: merge l1 l2 l) (x: X): merge l1 (x::l2) (x::l)
.
...
Proof.
  ...
  - reflexivity.
  - reflexivity.
  - simpl. simpl. rewrite rev_app_distr. rewrite <- IHpal. simpl. reflexivity.
Qed.
...
| r1 (x: X): repeats [x]
| r2 l (x: X) (H: repeats l): repeats (x::l)
. 
...
Proof.
  intros.
  split.
  - intros. inversion H. exists s4, s5. split.
    * reflexivity.
...
Proof.
  ...
  - (* 
    ...
    *) 
    ...
    exists s4. exists s5. 
    split. reflexivity. 
    split. rewrite H6 in H3. apply H3. 
    apply H4.
  - intros.
